...
<pre><code>function NameOfYourFunction {
    ...
    echo "&lt;script&gt;
          ...
          &lt;/script&gt;";
                        ...
                        <div class="question-10 question-red"><?php _e( 'What to do if this plugin crashed the website?', MCFUNC_TEXT ); ?></div>
                        <div class="answer-10"><?php _e( 'This plugin has a built-in functions for checking the custom code for syntax errors, duplicate functions names, and etc. But plugin is not perfect, so there are times when the entered custom code causes the error and white screen (WSOD). This is due to the fact that your custom code has a syntax error that this plugin could not detect. When this happens with you just do the following and all will be fine.', MCFUNC_TEXT ); ?>
                                              <ol class="custom-counter">
                                                    <li><?php _e( 'Via FTP, go to the plugin folder (in <code>wp-content/plugins/my-custom-functions/</code>).', MCFUNC_TEXT ); ?></li>
                                                    <li><?php _e( 'Rename the "START" file to "STOP". This will stop the execution of your custom code.', MCFUNC_TEXT ); ?></li>
                                                    <li><?php _e( 'Return to the plugin settings page and edit/fix your custom code that you entered before the crash.', MCFUNC_TEXT ); ?></li>
